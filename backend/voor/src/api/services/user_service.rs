...
pub async fn delete_user_account(
    ...
    let repo_ids = sqlx::query_scalar::<_, String>("SELECT id FROM repositories WHERE owner_id = $1")
        .bind(user_id)
        .fetch_all(&client.pool)
        .await
        .map_err(|error| format!("[ERROR] Failed to list repositories for user '{}': {}", user_id, error))?;
    ...
    sqlx::query("DELETE FROM repositories WHERE owner_id = $1")
        ...
        .execute(&client.pool)
        .await
        .map_err(|error| format!("[ERROR] Failed to delete repositories for user '{}': {}", user_id, error))?;

    sqlx::query("DELETE FROM stars WHERE user_id = $1")
        .bind(user_id)
        .execute(&client.pool)
        .await
        .map_err(|error| format!("[ERROR] Failed to delete user stars '{}': {}", user_id, error))?;

    sqlx::query("DELETE FROM repo_access_logs WHERE user_id = $1")
        .bind(user_id)
        .execute(&client.pool)
        .await
        .map_err(|error| format!("[ERROR] Failed to delete user access logs '{}': {}", user_id, error))?;

    sqlx::query("DELETE FROM commits_metadata WHERE author_id = $1")
        .bind(user_id)
        .execute(&client.pool)
        .await
        .map_err(|error| format!("[ERROR] Failed to delete user commit metadata '{}': {}", user_id, error))?;

    sqlx::query("DELETE FROM commits WHERE author_id = $1")
        .bind(user_id)
        .execute(&client.pool)
        .await
        .map_err(|error| format!("[ERROR] Failed to delete user commits '{}': {}", user_id, error))?;
